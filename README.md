...
lifted term (Lean; the Rocq ports of the latter two are recorded
deferrals, `delta.md §5.5–5.6`). `delta.md` records every decision
that diverges from the spec.
...
| Trust gate (Rocq) | 180 allowlisted theorems; 4 distinct axioms: `ClassicalDedekindReals.sig_forall_dec`, `FunctionalExtensionality.functional_extensionality_dep`, `classic`, `propositional_extensionality` | `tools/axiom_allowlist.json` (gated by `check_axioms.py` in the rocq-bridge job) |
