...
def runDispatchBrokerTwoBackendsFlow (rootDir : System.FilePath) : IO Unit := do
  ...
  -- The directive, as the fixture ships it: "1" first — a Tier 1
  -- Farkas witness (cvc4's internal closer, or cvc5's which now runs
  -- its internal closer before Tier 3 under a Farkas-first
  -- preference) is selected over cvc5's Tier 3 trace, in both orders.
  for (order, ms) in [("cvc4,cvc5", [cvc4Manifest, cvc5Manifest]),
                      ("cvc5,cvc4", [cvc5Manifest, cvc4Manifest])] do
  ...
  IO.println "OK dispatch_broker: the IR's tier_preference is honoured by the winner selection (fixture prefers \"1\": Tier 1 selected over cvc5's Tier 3, both orders)"
