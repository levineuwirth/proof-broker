...
(** End-to-end production path through the Alethe walker
    ...
    goal, dispatch to cvc5, verify the minted cert — then closes
    ONLY via [Alethe_walker.walk_proof_into_goal] on that cert, with
    NO [lia] fallback. Plain [proof_broker] wraps the walker in
    ...
    [propositional_extensionality] from the Prop-equality holes) —
    the same as the committed snapshot replay, but reached through the
