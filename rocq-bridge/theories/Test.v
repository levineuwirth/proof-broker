...
(** Alethe walker — snapshot test against a real cvc5 trace.
    ...
    below is the VERBATIM alethe-2024 output cvc5 minted for the
    goal [(n m : Z) (h1 : n + m = 10) (h3 : 0 <= m) |- n <= 10] —
    the same goal and same trace string Lean PR #52 pinned, reused
    here per the port plan. Walking it via [alethe_walker_test]
    exercises the Rocq walker against cvc5's actual output
    independently of live cvc5: CI needs no solver, and the trace
    text pins the walker against the real shape cvc5 emits. If cvc5
    changes its format (rule names, ordering, sugar) this test
    surfaces the drift immediately.

    What it exercises end-to-end, composing the whole arc:
    * cvc5's [(! expr :named @id)] syntax with [@p_X]
      back-references (parsed + expanded by the shared SDK);
    * byContra wrapping (R-9): the goal [n <= 10] is non-False, so
      the walker reduces it to [False] via [NNPP], exposing
      [~ (n <= 10)] for the trace's [assume a2] to match;
    * [equiv_pos2] (R-10, 3-literal tautology, no premises);
    * [hole] with TRUST_THEORY_REWRITE — re-derived independently
      of the tag (audit H1). Every hole here concludes a
      *propositional equality* of arithmetic facts, discharged via
