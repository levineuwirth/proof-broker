...
(* The end-to-end walker-strict live test (proof_broker_walker on
   example1) lives in its OWN file, TestWalkerLive.v: its
   [Print <name>] dumps the full reconstructed proof term, and dune
   truncates per-action output head+tail — keeping it here would evict
   sibling [Print Assumptions] blocks from the trust-footprint gate's
   view (the TestSnapshot.v lesson). *)
